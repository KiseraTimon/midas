...
public class DatabaseConduit {
    ...
    // Validates and, if valid, records the transaction. If invalid, discards with NO DB changes
    @Transactional
    public void validateAndRecord(Transaction tx) {
        ...
        if ("wilbur".equals(sender.getName())) {
            ...
            System.out.println("[Task4] Wilbur balance after tx (sender): " + b + " | floored=" + (int)Math.floor(b));
        }
        if ("waldorf".equals(recipient.getName())) {
            float b = recipient.getBalance();
            System.out.println("[Task4] Wilbur balance after tx (recipient): " + b + " | floored=" + (int)Math.floor(b));
        }
